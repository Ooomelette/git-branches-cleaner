...
const argv = require("yargs")
  .options({
    ...
    "older-than": {
      alias: "o",
      description:
        "Consider branches, which last commit date was older than chosen days ago",
      default: 90,
      type: "number",
    },
  })
...
function checkGitRemoteRepoExists() {
  if (
    !fs.existsSync(path.resolve(process.cwd(), ".git/refs/remotes")) ||
    !fs.existsSync(path.resolve(process.cwd(), ".git/refs/remotes/origin"))
  ) {
    logError(
      'Current git repository does not contain reference to "origin" remote repository.'
    );
    return -1;
  }
  return 0;
...
function getAllRemotes(excludedRegex) {
  const data = ex(
    `git for-each-ref --format='%(refname)#_#%(committerdate:unix)#_#%(authorname)#_#%(committername)#_#%(committerdate:relative)#_#%(objectname)' refs/remotes/origin`
  )
    .data.split("\n")
  ...
  return data
    ...
    .filter(
      (x) =>
        x["Branch Name"] &&
        (!excludedRegex || !x["Branch Name"].match(excludedRegex))
    );
}
...
function deleteOldBranches(branches) {
  console.log(
    "\x1b[33m###\x1b[0m \x1b[31mSTART OF DELETING REMOTE BRANCHES\x1b[0m \x1b[33m###\x1b[0m"
  );
  branches.forEach((x) => {
    const { data = "" } = deleteRemoteBranch(x["Branch Name"]) || {};
    console.log(`${data} commit: ${x.CommitId}`);
  });
}
...
(function main(argv) {
  let exitCode =
    checkGitRepoExists() || checkGitRemoteRepoExists() || gitFetchAll();
  if (exitCode) {
  ...
  if (oldBranches.length === 0) {
    console.log(
      `\x1b[31mTHERE ARE NO BRANCHES OLDER THAN \x1b[32m${argv.olderThan} DAY${
        argv.olderThan === 1 ? "" : "S"
      }\x1b[0m`
    );
    return;
